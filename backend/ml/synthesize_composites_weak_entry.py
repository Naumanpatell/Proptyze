"""
synthesize_composites_weak_entry.py — Paste door/lock patches onto wall backgrounds.

Output lands in:
  backend/ml/datasets/security/weak_entry_synthetic/train/images/
  backend/ml/datasets/security/weak_entry_synthetic/train/labels/

Labels use class 0 (weak_entry within sub-dataset).
Add weak_entry_synthetic to DATASET_CLASS_MAPS in merge_security.py when ready.

Run from: backend/ml/
    python synthesize_composites_weak_entry.py
"""
...
# Patch sources: doors (class 0) and locks (all classes = weak_entry)
DOOR_TRAIN = ML_DIR / "datasets" / "security" / "weak_entry_doors" / "train"
LOCK_TRAIN = ML_DIR / "datasets" / "security" / "weak_entry_locks" / "train"
...
OUT_DIR = ML_DIR / "datasets" / "security" / "weak_entry_synthetic" / "train"

BG_SIZE      = 640
SCALE_MIN    = 0.15   # doors are large objects
SCALE_MAX    = 0.55
PATCH_PAD    = 0.04
MAX_BG_TRIES = 20
...
def _extract_patches(src_dir: Path, class_idxs: set, label: str) -> list:
    img_dir = src_dir / "images"
    lbl_dir = src_dir / "labels"
    patches = []
    img_paths = sorted(img_dir.glob("*.jpg")) + sorted(img_dir.glob("*.png"))
    n = len(img_paths)
    print(f"  Extracting from {n} {label} images …")
    ...
    for img_path in img_paths:
        ...
        for line in lbl_path.read_text().strip().splitlines():
            ...
            if len(parts) != 5:
                continue
            if int(parts[0]) not in class_idxs:
                continue
            cx, cy, bw, bh = map(float, parts[1:])
...
def make_composite(bg: np.ndarray, patch: np.ndarray):
    ...
    alpha_j = random.uniform(0.8, 1.2)
    beta_j  = random.randint(-15, 15)
    ...
    feather = max(1, min(target_h, target_w) // 8)
    ks      = feather * 2 + 1
...
def main():
    ...
    print("=" * 60)
    print("  Proptyze — Synthetic Weak Entry Composites")
    print("=" * 60)
    print(f"  Target  : {args.target} composites")
    print(f"  Output  : {OUT_DIR}\n")

    patches  = _extract_patches(DOOR_TRAIN, {0}, "weak_entry_doors")
    patches += _extract_patches(LOCK_TRAIN, {0, 1}, "weak_entry_locks")
    if not patches:
        raise SystemExit("ERROR: no door/lock patches extracted")

    print(f"  Total patch pool: {len(patches)}")
    ...
    while generated < args.target and attempts < args.target * 10:
        ...
        if comp is None:
            continue
        cv2.imwrite(str(out_img / f"synth_entry_{generated:05d}.jpg"), comp, [cv2.IMWRITE_JPEG_QUALITY, 92])
        (out_lbl / f"synth_entry_{generated:05d}.txt").write_text(label)
        generated += 1
        if generated % 100 == 0:
            print(f"    [{generated}/{args.target}]")

    print(f"\n  Done — {generated} composites saved to {OUT_DIR}")
    print(f"  Next: add weak_entry_synthetic to merge_security.py, re-merge, train v2")
