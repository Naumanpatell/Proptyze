"""
synthesize_composites_damp.py — Paste damp patches onto clean background crops
...
DAMP_TRAIN = ML_DIR / "datasets" / "condition" / "damp" / "train"
# damp dataset class index for "damp" (crack=0, damp=1, fungus=2, overlap=3)
DAMP_CLASS_IDX = 1
...
OUT_DIR    = ML_DIR / "datasets" / "condition" / "damp_synthetic" / "train"

BG_SIZE      = 640
SCALE_MIN    = 0.08   # damp patches tend to be larger than mould
SCALE_MAX    = 0.35
PATCH_PAD    = 0.05
MAX_BG_TRIES = 20
...
def build_patch_pool(damp_train: Path) -> list:
    img_dir = damp_train / "images"
    lbl_dir = damp_train / "labels"
    patches = []
    img_paths = sorted(img_dir.glob("*.jpg"))
    n = len(img_paths)
    print(f"  Building patch pool from {n} damp/train images …")
...
def make_composite(bg: np.ndarray, patch: np.ndarray):
    ...
    # class 0 = damp (local sub-dataset ID)
    cx_n  = (x1 + x2) / 2 / bg_w
    cy_n  = (y1 + y2) / 2 / bg_h
    w_n   = target_w / bg_w
    h_n   = target_h / bg_h
...
def main():
    ...
    print("=" * 60)
    print("  Proptyze — Synthetic Damp Composites")
    print("=" * 60)
    print(f"  Target  : {args.target} composites")
    print(f"  Seed    : {args.seed}")
    print(f"  Output  : {OUT_DIR}\n")

    patches = build_patch_pool(DAMP_TRAIN)
    if not patches:
        raise SystemExit("ERROR: no damp patches extracted — check damp/train/labels")
    ...
    while generated < args.target and attempts < max_attempts:
        ...
        img_name = f"synth_damp_{generated:05d}.jpg"
        lbl_name = f"synth_damp_{generated:05d}.txt"
    ...
    if scales_pct:
        ...
        pct_bins = [0] * 5
        for s in scales_pct:
            if   s < 10: pct_bins[0] += 1
            elif s < 15: pct_bins[1] += 1
            elif s < 20: pct_bins[2] += 1
            elif s < 28: pct_bins[3] += 1
            else:        pct_bins[4] += 1

        print(f"\n  Patch width as % of image width:")
        labels = ["<10%", "10–15%", "15–20%", "20–28%", "28–35%+"]
        ...
        print(f"  Then add damp_synthetic to DATASET_CLASS_MAPS in merge_condition.py")
        print(f"  and re-run: python merge_condition.py && python train.py --model condition")
