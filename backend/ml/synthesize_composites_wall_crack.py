...
WALL_CRACK_TRAIN = ML_DIR / "datasets" / "condition" / "wall_crack" / "train"
# wall_crack dataset: DiagonalCrack=0, HorizontalCrack=1, VerticalCrack=2 — all are cracks
CRACK_CLASS_IDXS = {0, 1, 2}
...
OUT_DIR = ML_DIR / "datasets" / "condition" / "wall_crack_synthetic" / "train"

BG_SIZE      = 640
SCALE_MIN    = 0.05   # cracks are thin — keep them smaller than damp patches
SCALE_MAX    = 0.25
PATCH_PAD    = 0.04
MAX_BG_TRIES = 20
...
def build_patch_pool(crack_train: Path) -> list:
    img_dir = crack_train / "images"
    lbl_dir = crack_train / "labels"
    patches = []
    img_paths = sorted(img_dir.glob("*.jpg"))
    n = len(img_paths)
    print(f"  Building patch pool from {n} wall_crack/train images …")
    ...
    for i, img_path in enumerate(img_paths):
        ...
        for line in lbl_path.read_text().strip().splitlines():
            ...
            if len(parts) != 5:
                continue
            cls_idx = int(parts[0])
            if cls_idx not in CRACK_CLASS_IDXS:
                continue
            cx, cy, bw, bh = map(float, parts[1:])
            ...
            if x2 - x1 >= 8 and y2 - y1 >= 8:
                ...
                if crop.shape[1] > 200:
                    scale_f = 200 / crop.shape[1]
                    crop = cv2.resize(crop, (200, max(8, int(crop.shape[0] * scale_f))), interpolation=cv2.INTER_AREA)
...
def make_composite(bg: np.ndarray, patch: np.ndarray):
    ...
    # class 0 = wall_crack (local sub-dataset ID)
    cx_n  = (x1 + x2) / 2 / bg_w
    cy_n  = (y1 + y2) / 2 / bg_h
    w_n   = target_w / bg_w
    h_n   = target_h / bg_h
...
def main():
    ...
    print("=" * 60)
    print("  Proptyze — Synthetic Wall Crack Composites")
    print("=" * 60)
    print(f"  Target  : {args.target} composites")
    print(f"  Seed    : {args.seed}")
    print(f"  Output  : {OUT_DIR}\n")

    patches = build_patch_pool(WALL_CRACK_TRAIN)
    if not patches:
        raise SystemExit("ERROR: no crack patches extracted — check wall_crack/train/labels")
    ...
    while generated < args.target and attempts < max_attempts:
        ...
        img_name = f"synth_crack_{generated:05d}.jpg"
        lbl_name = f"synth_crack_{generated:05d}.txt"
    ...
    if scales_pct:
        ...
        for s in scales_pct:
            if   s < 8:  pct_bins[0] += 1
            elif s < 12: pct_bins[1] += 1
            elif s < 16: pct_bins[2] += 1
            elif s < 20: pct_bins[3] += 1
            else:        pct_bins[4] += 1

        print(f"\n  Patch width as % of image width:")
        labels = ["<8%", "8–12%", "12–16%", "16–20%", "20–25%+"]
        ...
        print(f"\n  Next step: add wall_crack_synthetic to DATASET_CLASS_MAPS in merge_condition.py")
        print(f"  and re-run: python merge_condition.py && python train.py --model condition --weights runs/condition/v3/weights/best.pt --name v4")
