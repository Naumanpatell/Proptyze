...
DATASETS_DIR = ML_DIR / "datasets" / "condition"
MERGED_DIR   = DATASETS_DIR / "merged"

CONDITION_CLASSES = ["wall_crack", "damp", "mould", "peeling_paint", "broken_fixture"]
TARGET_IDX        = {cls: i for i, cls in enumerate(CONDITION_CLASSES)}

# Keys must match folder names under datasets/condition/
...
DATASET_CLASS_MAPS: dict[str, dict[str, str | None]] = {
    ...
    "damp": {
        "crack":  "wall_crack",
        "damp":   "damp",
        "fungus": "mould",     # fungus ≈ mould for our purposes
        "*":      None,
    },
    "mould": {
        "*": "mould",
...
def merge_one(dataset_name: str, class_map: dict[str, str | None]) -> dict[str, int]:
    ...
    mapping_str = {
        src_names[i]: CONDITION_CLASSES[v] if v is not None else "DISCARD"
...
def merge() -> Path:
    ...
    yaml_data = {
        ...
        "train": "train/images",
        "val":   "valid/images",
        "nc":    len(CONDITION_CLASSES),
        "names": CONDITION_CLASSES,
    }
...
if __name__ == "__main__":
    print("=" * 60)
    print("  Merging condition datasets")
    print("=" * 60)
    print(f"  Classes : {CONDITION_CLASSES}\n")
    merge()
    print("\nDone. Run: python train.py --model condition")
